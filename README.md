# Andrews Ferreira

**Application Security • Offensive Security • Security Engineering**

Building security tools and translating technical findings into business risk.

[LinkedIn](https://www.linkedin.com/in/andrewslferreira) •
[Medium](https://medium.com/@andrewsferreira) •
[Hack The Box](https://profile.hackthebox.com/profile/019dd05a-92c2-7383-8a72-144282de471e) •
[GitHub](https://github.com/andrewsferreira)

---

## Practical Security Focus

- Web Application Security
- API Security
- Authorization and Business Logic
- Attack-Path Analysis
- Security Automation
- Threat Modeling and Secure Architecture

---

## Certifications

- **API-RTA — API Red Team Analyst**, CyberWarFare Labs
- **WEB-RTA — Certified Web Red Team Analyst**, CyberWarFare Labs

---

**Featured Projects**

🛡️ **[LLM Security Testing Framework](https://github.com/andrewsferreira/llm-security-testing-framework)**

AI Security testing framework for LLM-powered applications.

`Python` · `FastAPI` · `Docker` · `GitHub Actions`

📊 **[RiskBridge](https://github.com/andrewsferreira/rbvm-platform)**

Risk-Based Vulnerability & Exposure Management platform inspired by security operations.

`JavaScript` · `HTML` · `CSS` · `GitHub Pages`

⚔️ **[ReconForge](https://github.com/andrewsferreira/reconforge)**

Modular reconnaissance framework for professional security assessments.

`Python` · `PyYAML` · `Cryptography` · `MCP`

---

**Expertise**

Threat Modeling • Security Architecture • Secure SDLC • DevSecOps • Security Automation • 
Risk-Based Vulnerability Management • Offensive Security
